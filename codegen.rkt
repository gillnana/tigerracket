...
(define (gen-code ir [cur-block #f])
  (match ir
    ...
    [(ref-ins src1 src2)
     ...
     #;(ln "sw " TEMP0 COMMA "(" TEMP1 ")")
     (lnstore src1 TEMP0 cur-block)]
...
;     (ln "lw " TEMP2 COMMA (get-offset src2 temps) "($sp)")
    ...
    [(array-allocate-ins src1 dest) (error "fail")] ; as presently generated by intermediate, requires some changes to use array_alloc or could use alloc_block
...
(define (lnload src-loc dest-reg cur-block)
  ...
  (match src-loc
   ...
   #; [(param-loc sym num)
    ...
    [(mem-loc sym size) (error 'do-this)] ; we will need an environment to keep track of all the random stuff on the heap
    )
